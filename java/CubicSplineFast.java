...
public class CubicSplineFast{
    	...
    	public void calcDeriv(){
	    	...
	    	for(int i=1;i<=this.nPoints-2;i++){
		    	...
		    	p=sig*this.d2ydx2[i-1]+2.0D;
		    	this.d2ydx2[i]=(sig-1.0D)/p;
		    	u[i]=(this.y[i+1]-this.y[i])/(this.x[i+1]-this.x[i]) - (this.y[i]-this.y[i-1])/(this.x[i]-this.x[i-1]);
		    	u[i]=(6.0D*u[i]/(this.x[i+1]-this.x[i-1])-sig*u[i-1])/p;
	    	}

		    qn=un=0.0D;
	    	this.d2ydx2[this.nPoints-1]=(un-qn*u[this.nPoints-2])/(qn*this.d2ydx2[this.nPoints-2]+1.0D);
    ...
    public double lastX() {
        ...
        double h=0.0D,b=0.0D,a=0.0D, yy=0.0D;
	    	...
	    	if (h == 0.0){
	        	...
	        	a=(this.x[khi]-xx)/h;
	        	b=(xx-this.x[klo])/h;
	        	yy=a*this.y[klo]+b*this.y[khi]+((a*a*a-a)*this.d2ydx2[klo]+(b*b*b-b)*this.d2ydx2[khi])*(h*h)/6.0D;
	    	}
	    	return yy;
    	}
}
